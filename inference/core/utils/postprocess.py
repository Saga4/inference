...
def mask2poly(mask: np.ndarray) -> np.ndarray:
    ...
    """
    # Using cv2.findContours directly on the original mask
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if contours:
        # Find the largest contour by directly comparing based on lengths
        largest_contour = max(contours, key=len)
        contours_array = np.array(largest_contour).reshape(-1, 2).astype(np.float32)
    else:
        contours_array = np.zeros((0, 2), dtype=np.float32)
